...
| **Fase 2** | Transaccionalidad (RPC Atómico) & Sincronización Offline | 🔴 **En Curso** | Alta |
| **Fase 3** | Descomposición Monolítica & Limpieza de Deuda Restaurante | 🟡 **Pendiente** | Media-Alta |
| **Fase 4** | Consolidación de Modelos Duales de DB & Reglas Hardcodeadas | 🟡 **Pendiente** | Media-Alta |
...
## 🔴 Fase 2: Confiabilidad Transaccional & Sincronización Offline (PRÓXIMO PASO)

### 2.1 RPC Transaccional Atómico de Ventas (`process_retail_sale_transaction`)
- [ ] **Migración SQL PostgreSQL**:
  - Crear función `process_retail_sale_transaction(p_sale jsonb, p_items jsonb, p_options jsonb DEFAULT '{}') RETURNS jsonb`.
  - Operación en una sola transacción atómica (`BEGIN ... COMMIT`):
    1. Validar tenant (`organization_id`).
    2. Insertar encabezado en `retail_sales`.
    3. Insertar partidas en `retail_sale_items`.
    4. Actualizar acumulado de compras del cliente (`clients.total_spent`).
    5. Descontar inventario en `retail_products`.
    6. Registrar auditoría en `audit_logs`.
- [ ] **Integración en Frontend**:
  - Actualizar `createRetailSale` en `src/services/supabaseService.ts` para ejecutar el RPC de forma atómica.

### 2.2 Unificación de Capas Offline
- [ ] **Deprecar `offlineDBService.ts`** (IndexedDB legacy 'TPVSolutions').
- [ ] **Consolidar `offlineStorage.ts`**:
  - Motor IndexedDB principal (`idb`) con base `reisbloc_sync_queue_db`.
  - Cola única `sync_queue` con estados formales: `pending`, `syncing`, `synced`, `failed`.
  - Mecanismo de reintento exponencial y deduplicación por UUID de cliente.
...
- [ ] Reemplazar llamadas a `clearDraftForTable` por `clearDraftForTicket`.
...
- [ ] **`shiftService.ts`**:
  - Cambiar consultas de `sales` a `retail_sales` para que los cortes de turno calculen el monto esperado real.
- [ ] **`Reports.tsx` y `Analytics.tsx`**:
  - Migrar selectores de `products` y `sales` a `retail_products` y `retail_sales`.
