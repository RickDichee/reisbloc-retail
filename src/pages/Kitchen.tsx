    ...
    const unsubscribe = supabaseService.subscribeToActiveOrders(
      (data) => {
        ...
        if (newOrders.length > 0 && prevOrderIds.length > 0) {
          ...
          showOrderToast('📦 Nuevo Pedido', `${newOrders.length} pedido(s) listo(s) para empaque`, 5000)
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-rb-canvas">
        <div className="text-center">
          <ChefHat className="mx-auto mb-4 text-blue-600 animate-pulse" size={48} />
          <p className="text-slate-600 text-lg font-bold">Cargando pedidos...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-rb-canvas relative">
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-7xl mx-auto px-4 pt-8">
          <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-3xl shadow-lg overflow-hidden">
            <div className="px-6 py-6 sm:px-8 flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-3">
                <div className="p-4 bg-white/20 rounded-2xl backdrop-blur-sm">
                  <ChefHat size={32} />
                </div>
                <div>
                  <p className="text-sm text-blue-100 uppercase tracking-tighter font-black">Área de Bodega / Almacén</p>
                  <h1 className="text-2xl sm:text-3xl font-black tracking-tight uppercase">Despacho</h1>
                  <p className="text-blue-100 mt-1 font-bold opacity-80">GESTIÓN DE PEDIDOS EN VIVO</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                {newOrdersCount > 0 && (
                  <button
                    ...
                    className="relative px-6 py-3 bg-red-500 hover:bg-red-600 rounded-2xl font-black flex items-center gap-3 animate-pulse shadow-xl transition-all hover:scale-105 active:scale-95"
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredOrders.map((order) => {
                const statusConfig = {
                  ...
                  sent: { label: 'En bodega', color: 'bg-orange-900 text-orange-200', border: 'border-orange-600' },
                  ready: { label: 'Empacado', color: 'bg-green-900 text-green-200', border: 'border-green-600' },
                  served: { label: 'Entregado', color: 'bg-blue-900 text-blue-200', border: 'border-blue-600' },
                      ...
                      {order.items?.map((item, i) => (
                        <div key={i} className="flex justify-between items-center">
                          <div>
                            ...
                            <p className="text-slate-500 font-medium text-xs">${item.unitPrice.toFixed(2)}</p>
                          </div>
