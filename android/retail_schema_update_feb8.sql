...
CREATE OR REPLACE FUNCTION public.enforce_single_primary_admin() RETURNS TRIGGER AS $$
BEGIN
...
CREATE POLICY "Manager can use pos all access" ON orders FOR ALL TO authenticated 
USING (
...
CREATE POLICY "Manager can manage products" ON products FOR ALL TO authenticated 
USING (
  organization_id = (SELECT organization_id FROM users WHERE id = auth.uid())
...
CREATE POLICY "Manager can view sales and billing" ON sales FOR SELECT TO authenticated 
USING (
  organization_id = (SELECT organization_id FROM users WHERE id = auth.uid())
);
...
-- =========================================================================
CREATE OR REPLACE FUNCTION public.notify_new_device_pending() RETURNS TRIGGER AS $$
DECLARE
  v_admin_id UUID;
BEGIN
  ...
  IF NEW.is_approved = false THEN
    ...
    IF v_admin_id IS NOT NULL THEN
      INSERT INTO public.notifications (
        user_id, title, body, type, priority, data, read, created_at
      ) VALUES (
        v_admin_id,
        '🔐 Nuevo Dispositivo: Aprobación Pendiente',
        'El usuario ' || (SELECT name FROM public.users WHERE id = NEW.user_id) || ' necesita aprobación de dispositivo para acceder.',
        'device_approval',
        'high',
        jsonb_build_object(
          'device_id', NEW.id,
...
-- =========================================================================
CREATE OR REPLACE FUNCTION public.audit_user_creation() RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.audit_logs (
    user_id, action, table_name, record_id, changes, created_at
  ) VALUES (
